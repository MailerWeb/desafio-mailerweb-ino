...
export default function EditBooking() {
  ...
  const fetchBooking = async () => {
    try {
      ...
      setFormData({
        title: booking.title,
        start_at: new Date(booking.start_at).toISOString().slice(0, 16),
        end_at: new Date(booking.end_at).toISOString().slice(0, 16),
        participants: booking.participants?.join(', ') || '',
      })
    } catch (error) {
      const message = error.response?.data?.detail || 'Erro ao carregar reserva'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setAlert({
        type: 'success',
        message: 'Reserva atualizada com sucesso! Notificações estão sendo enviadas...',
      })
  ...
  return (
    <div className="container">
      <div style={{ marginTop: '20px' }}>
        <h1 style={{ color: 'white', marginBottom: '20px' }}>Editar Reserva ✏️</h1>
        ...
        <div className="card">
          ...
          <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#f0f7ff', borderRadius: '5px' }}>
            <p style={{ fontSize: '14px', color: '#333' }}>
              ...
              <br />
              • Os participantes serão notificados sobre as alterações
            </p>
          </div>
        </div>
      </div>
